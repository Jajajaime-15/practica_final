...
export class PrestamoRepository{
    ...
    async buscarPorId(id) {
        const { data, error } = await supabase // peticion 'select' a supabase con condicion (solo queremos un prestamo)
            .from('prestamos')
            .select('*') // join para que muestre el titulo del libro prestado y el nombre del usuario que hace el prestamo
    ...
    async buscarTodos() {
        const { data, error } = await supabase // peticion 'select' a supabase sin condicion (queremos todos los prestamos)
            .from('prestamos')
            .select('*') // join para mostrar el titulo del libro prestado y el usuario que hace el prestamo
            .order('created_at', { ascending: false }); // mostramos los prestamos ordenados de mas reciente a mas antiguo

        if (error) throw error;
        return data.map(item => new Prestamo(item)); // convertimos cada registro del array en objeto Prestamo
    }
